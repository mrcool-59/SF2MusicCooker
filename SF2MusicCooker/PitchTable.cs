    ...
    public sealed class PitchTable
    {
        ...
        */
        ...
        private readonly struct Entry
        {
            ...
            public Entry(byte note, ushort value, int frequency)
            {
                Note = note;
                Value = value;
                Frequency = frequency;
        ...
        */
        ...
        public byte[] GetNotes(bool psg, Predicate<byte> filter = null)
        {
            ...
            foreach (Entry entry in source) { if (filter(entry.Note)) notes.Add(entry.Note); }
        ...
        {
            string asm = BuildFrequencies("YM_FREQUENCIES", "YM Frequency values for each driver note value from note 0 to $53", _notes);
            File.WriteAllText(Path.Combine(path, "ym_frequencies-standard.asm"), asm);
            Console.WriteLine("> Wrote 'ym_frequencies-standard.asm' file!");
        }
        ...
        {
            string asm = BuildFrequencies("PSG_FREQUENCIES", "PSG Frequency values for each driver note value from note 0 to $3F", _psgNotes);
            File.WriteAllText(Path.Combine(path, "psg_frequencies-standard.asm"), asm);
            Console.WriteLine("> Wrote 'psg_frequencies-standard' file!");
        }

        private string BuildFrequencies(string label, string comment, Entry[] notes)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine();
            sb.Append("; ");
            sb.Append(comment);
            sb.AppendLine(" (this file has been generated by an automated tool)");
            sb.AppendLine();
            sb.Append(label);
            sb.Append(':');
            sb.AppendLine();
            ...
            {
                sb.Append("    dw ");
                sb.Append(Tools.Hex2ASM(note.Value));
                sb.AppendLine();
            }
            return sb.ToString();
        }
