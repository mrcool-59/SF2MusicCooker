...
namespace SF2MusicCooker.Furnace
{
    public static class FeatureInterpreter
    {
        ...
        /// <summary>
        /// Parse a "MA" Furnace feature (macro) attached to a PSG instrument and return the envelope if there is one.
        /// </summary>
        public static Envelope ParseFurnacePSGMacroLevels(byte[] data)
        {
            if (data[0] != 8 || data[1] != 0 || data.Length < 10)
                throw new FormatException("Data payload must contain at least 10 bytes, with a 8 bytes header");

            byte code = data[2];
            if (code == 0xFF) return null;
            else if (code != 0x00) throw new NotSupportedException("Unsupported PSG instrument macro code: " + code);

            byte length = data[3];

            _ = data[4]; // Loop (ignored)

            byte releaseTick = data[5];

            _ = data[6]; // Mode (ignored)

            byte type = data[7]; // Type (ignored)
            if (type > 0x01) return null;

            _ = data[8]; // Delay (ignored)

            _ = data[9]; // Speed (ignored)
            ...
            {
                if (i > releaseTick || i == length - 1)
                    release.Add(data[10 + i]);
                else
                    attack.Add(data[10 + i]);
            }

            if (attack.Count > 0 && release.Count > 0)
                return new Envelope(attack.ToArray(), release.ToArray());
            else
                return null;
        }
    }
